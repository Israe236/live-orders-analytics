...
def test_score_separates_false_alerts_from_detected_incidents() -> None:
    ...
    assert result.false_alerts == {"cancellation_rate": 1, "revenue_drop": 1}
    assert [(d.kind, d.detected, d.delay_s) for d in result.detections] == [
        ("payment_outage", True, 100),
        ("traffic_drop", False, None),
    ]
...
def test_an_injected_payment_outage_is_detected_on_steady_traffic() -> None:
    ...
    traffic = simulate(config, start=START, seconds=2_400, incidents=[incident])

    thresholds = Thresholds(window_minutes=3)
    ...
    assert result.false_alerts == {"cancellation_rate": 0, "revenue_drop": 0}
